...
- Nav labels — the real integration is now the primary **"Odoo"** link
  (badge: Live); the old mock is relabeled **"Odoo (Demo data)"** and its
  banner/status text now always says it's a fixed sample set, so it can
  never be mistaken for a live connection again.
