...
- **Dashboard demo data** — there isn't any: Overview, Odoo Live and Audit
  log all read live data from your connected Odoo through the Cloudflare
  Worker proxy (see `ODOO_SETUP.md`, and `cloudflare-worker.js` at the repo
  root). Nothing renders in those tabs until Odoo is connected in
  Settings → Odoo.
...
- **Settings** — open `settings.html` (or press Ctrl/⌘ + , on any page): theme (system / light / dark), accent color, motion, install status, offline status, update check, clear saved files, export / import / reset. One store, `js/config.js` (`window.DV`), drives every page.
...
- **Releasing an update** — bump `VERSION` in `sw.js`. Users get a "Reload to update" button in Settings. New CSS/JS files are cached automatically the first time they load.
