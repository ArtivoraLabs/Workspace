...
// js/shell.js is the single owner of the #themeToggleBtn click handler (it then
// calls window.__studioApplyTheme so Studio's icon/charts mirror the change) —
// inline it too so "click the theme button" exercises the same real wiring the
// browser uses, instead of relying on a duplicate handler that used to also
// live in studio-ui.js (removed: the two together caused a double-toggle bug).
html = html.replace('<script src="js/shell.js"></script>', () => inline('js/shell.js'));
